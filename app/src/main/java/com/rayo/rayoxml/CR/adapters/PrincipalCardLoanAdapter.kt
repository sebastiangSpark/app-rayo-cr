...
class PrincipalCardLoanAdapter(
    ...
    class LoanViewHolder(private val binding: FragmentPrincipalCardLoanBinding,
        ...
        fun bind(item: PrincipalLoanCardData) {
            ...
            binding.cardDate.text = formatDate(item.date)
            ...
            when (item.state) {
                "Pendiente" -> {
                    ...
                    // Desactivar campo de fecha
                    binding.cardDate.visibility = View.GONE
                    binding.cardDateIcon?.visibility = View.GONE
                }
                "En Revisión" -> {
                    backgroundDrawable.setColor(ContextCompat.getColor(context, R.color.Matisse_100))
                    binding.cardState.setTextColor(ContextCompat.getColor(context, R.color.Matisse_700))
                    binding.buttonOpenPaymentInstallment.visibility = View.INVISIBLE
                    // Cambiar ícono
                    binding.cardPaymentIcon?.setImageResource(R.drawable.ic_count_fees)
                }
                "Cancelado" -> {
    ...
    companion object {
        fun formatDate(dateString: String): String {
            return try {
                ...
                "Próximo pago: " + outputFormat.format(date ?: cleanDate)
            } catch (e: Exception) {
