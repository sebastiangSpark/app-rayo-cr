...
import com.rayo.rayoxml.co.services.Loan.LoanRenewalInfoLoadResponse
...
import com.rayo.rayoxml.co.services.Loan.LoanStepFivePlusSubmitRequest
...
import com.rayo.rayoxml.co.ui.dialogs.BankDialogFragment
...
class BankDisbursementFragment(/*private val viewModel: UserViewModel*/) : Fragment() {
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        binding.btnSkip.setOnClickListener {
            when (parentFragment) {
                ...
                is RenewalFragment -> findNavController().navigate(R.id.bankDisbursementFragment)
            }
        }

        // Botón corregir datos bancarios
        binding.btnUpdateAccount.setOnClickListener {
            Log.d("BankDisbursementFragment", "Actualizar datos bancarios")
            val bundle = bundleOf("step" to 1)
        ...
        binding.backToolbarIcon.setOnClickListener {
            goBack()
            /*Log.d("BankDisbursementFragment", "Ir atrás")
        ...
        /*viewModel.phone.observe(viewLifecycleOwner) { data ->
            Log.d("BankDisbursementFragment", "User phone: $data")
            if (data != null) {
                phone = data
            }
        }*/
        ...
        renewalViewModel.totalLoans.observe(viewLifecycleOwner) { data ->
            data?.let {
                totalLoans = it
                Log.d("BankDisbursementFragment", "Total préstamos: ${totalLoans}")
            }
        }
        ...
        renewalViewModel.loanType.observe(viewLifecycleOwner) { data ->
            data?.let {
                loanType = it
                Log.d("BankDisbursementFragment", "Tipo de préstamo: ${loanType}")
            }
        }
        ...
        renewalViewModel.technologyCheck.observe(viewLifecycleOwner) { option ->
            option?.let {
                ...
                Log.d("BankDisbursementFragment", "Datos de check de tecnología: ${technologyEnabled}")
            }
        }
        ...
        renewalViewModel.infoResponse.observe(viewLifecycleOwner) { data ->
            data?.let {
                ...
                Log.d("BankDisbursementFragment", "Datos de info de renovación (response): ${loanRenewalInfoLoadResponse}")
                // Deshabilitar botón datos bancarios
        ...
        renewalViewModel.infoRpResponse.observe(viewLifecycleOwner) { data ->
            data?.let {
                ...
                Log.d("BankDisbursementFragment", "Datos de info de renovación RP (response): ${loanRpRenewalInfoLoadResponse}")
                // Deshabilitar botón datos bancarios
                binding.btnUpdateAccount.visibility = View.INVISIBLE
            }
        }
        renewalViewModel.loanTerm.observe(viewLifecycleOwner) { data ->
            data?.let {
                loanTerm = it
                Log.d("BankDisbursementFragment", "Datos de plazo: ${loanTerm}")
            }
        }
        ...
        parentFragmentManager.setFragmentResultListener("bankDialogResult", viewLifecycleOwner) { _, bundle ->
            ...
            if( loanType.isNullOrEmpty() || totalLoans == 0){
    ...
    private fun goBack() {
        if(loanTerm == 0){
            Log.d("BankDisbursementFragment", "Ir atrás crear préstamo")
            val bundle = bundleOf("step" to 4)
            findNavController().navigate(R.id.formFragment, bundle)
        }else{
            Log.d("BankDisbursementFragment", "Ir atrás renovación préstamo $loanType")
    ...
    private suspend fun submitLoanMini(){
        if( !::loanRenewalInfoLoadResponse.isInitialized){
            Log.d("BankDisbursementFragment", "loanRenewalInfoLoadResponse no inicializada")
            return
        }
        ...
        try {
            ...
            Log.d("BankDisbursementFragment", "Loan renewal info load: ${infoSubmitResponse}")

            if(infoSubmitResponse != null){
                if(infoSubmitResponse.solicitud.codigo == "200"){
                    Log.d("BankDisbursementFragment", "Solicitud información OK")

                    // Continuar a sigte pantalla
                    Log.d("BankDisbursementFragment", "Cargando pantalla de confirmación de préstamo")
                    ...
                    if (infoSubmitResponse != null) {
                        if(infoSubmitResponse.solicitud.codigo == "200" && infoSubmitResponse.solicitud.result == SUCCESSFUL_FORM_RESULT){
                            // Mostrar pantalla de confirmación
                            Log.d("BankDisbursementFragment", "Préstamo renovado: ${infoSubmitResponse.solicitud.prestamo}")
                            val bundle = Bundle().apply {
                                putSerializable("outcome", LoanOutcome.SUCCESS_BANK)
                            }
                            findNavController().navigate(R.id.loanOutcomeFragment, bundle)
                    ...
                    Log.d("BankDisbursementFragment", "Error Loan renewal info load: ${infoSubmitResponse}")
                ...
                Log.d("BankDisbursementFragment", "infoLoadResponse nulo")
            }
    ...
    private suspend fun submitLoanRP(){
        ...
        try {
            ...
            Log.d("BankDisbursementFragment", "Loan rp renewal info load: ${infoSubmitResponse}")
            ...
            if(infoSubmitResponse != null){
                if(infoSubmitResponse.solicitud.codigo == "200"){
                    ...
                    // Continuar a sigte pantalla
                    Log.d("BankDisbursementFragment", "Cargando pantalla de confirmación de préstamo RP")
                    ...
                    if (infoSubmitResponse.solicitud.result == SUCCESSFUL_FORM_RESULT) {
                        // Mostrar pantalla de confirmación
                        Log.d("BankDisbursementFragment", "Préstamo RP renovado: ${infoSubmitResponse.solicitud.prestamo}")
                        val bundle = Bundle().apply {
                            putSerializable("outcome", LoanOutcome.SUCCESS_BANK)
                        }
                        findNavController().navigate(R.id.loanOutcomeFragment, bundle)
