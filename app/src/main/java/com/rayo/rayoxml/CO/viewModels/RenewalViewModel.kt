...
import com.rayo.rayoxml.co.services.Loan.LoanRenewalProposalSubmitResponse
import com.rayo.rayoxml.co.services.Loan.LoanStepFiveLoadRequest
import com.rayo.rayoxml.co.services.Loan.LoanStepFiveLoadResponse
...
class RenewalViewModel : ViewModel() {
    ...
    // Número de préstamos
    private val _totalLoans = MutableLiveData<Int?>()
    val totalLoans: LiveData<Int?> get() = _totalLoans

    fun setTotalLoans(total: Int) {
        _totalLoans.value = total
    }

    // Formulario PLP paso 1
    private val _plpStep1Response = MutableLiveData<SolicitudPlpStep1?>()
    val plpStep1Response: LiveData<SolicitudPlpStep1?> get() = _plpStep1Response

    fun setPlpStep1Response(data: SolicitudPlpStep1) {
        _plpStep1Response.value = data
    }
