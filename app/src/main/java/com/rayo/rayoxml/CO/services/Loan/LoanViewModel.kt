...
class LoanStepOneViewModel(private val repository: LoanRepository) : ViewModel() {
    ...
    // Intentos verificación SMS
    private val _verificationAttempts = MutableLiveData<Int?>().apply { value = 0 }
    val verificationAttempts: LiveData<Int?> get() = _verificationAttempts

    fun incrementVerificationAttempts() {
        _verificationAttempts.value = (_verificationAttempts.value ?: 0) + 1

        /*_verificationAttempts.value?.let { currentValue ->
            _verificationAttempts.postValue(currentValue + 1)
        }*/
    }

    // Intentos verificación preguntas
    private val _questionsVerificationAttempts = MutableLiveData<Int?>().apply { value = 0 }
    val questionsVerificationAttempts: LiveData<Int?> get() = _questionsVerificationAttempts

    fun incrementQuestionsVerificationAttempts() {
        _questionsVerificationAttempts.value = (_questionsVerificationAttempts.value ?: 0) + 1

        /*_questionsVerificationAttempts.value?.let { currentValue ->
            _questionsVerificationAttempts.postValue(currentValue + 1)
        }*/
    }

    // Inicializar contadores
    /*fun initVerificationAttempts() {
        if(_verificationAttempts.value != 0){
            println("Inicializando valores")
            _verificationAttempts.value = 0
            _questionsVerificationAttempts.value = 0
        }else{
            println("Ya fueron inicializados")
        }
    }*/
